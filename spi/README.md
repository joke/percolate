...
The module depends only on the JDK and the relocated `javapoet` (`io.github.joke.percolate.lib.javapoet.CodeBlock` is part of the strategy contract). It does NOT depend on `annotations` or `processor`.
