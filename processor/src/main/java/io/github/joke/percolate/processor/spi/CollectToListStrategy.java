...
public final class CollectToListStrategy implements TypeTransformStrategy {
    ...
    public Optional<TransformProposal> canProduce(final TypeMirror sourceType, final TypeMirror targetType,
        ...
        if (!types.isSameType(erasedTarget, types.erasure(listElement.asType()))) {
            return Optional.empty();
        }
        ...
        return Optional.of(new TransformProposal(
                streamType,
                targetType,
                input -> CodeBlock.of("$L.collect($T.toList())", input, Collectors.class),
                this));
    }
}
