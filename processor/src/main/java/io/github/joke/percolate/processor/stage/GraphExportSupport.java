...
class GraphExportSupport {
    ...
    private static <V, E> String exportToString(
        ...
        switch (format.toLowerCase(Locale.ROOT)) {
            case "graphml":
                ...
                mlExporter.registerAttribute(
                        "label", GraphMLExporter.AttributeCategory.NODE, AttributeType.STRING, "");
                discoverEdgeAttributes(graph, edgeAttributeProvider)
                        .forEach((name, type) -> mlExporter.registerAttribute(
                                name, GraphMLExporter.AttributeCategory.EDGE, type, ""));
    ...
    private static <V, E> Map<String, AttributeType> discoverEdgeAttributes(
        ...
        for (final var edge : graph.edgeSet()) {
            edgeAttributeProvider.apply(edge).forEach((name, attr) ->
                    result.putIfAbsent(name, attr.getType()));
        }
        return result;
    }
