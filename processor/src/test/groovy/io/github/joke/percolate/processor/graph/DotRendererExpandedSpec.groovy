...
class DotRendererExpandedSpec extends Specification {
    ...
    def 'REALISED edge renders with solid style and heavy penwidth'() {
        given:
    ...
    def 'SUB_SEED edge renders with solid style and gray color'() {
        given:
        def renderer = new DotRenderer()
        def graph = new MapperGraph()
        def s = scope('map()')
    ...
    def 'SUB_SEED edge label is SUB_SEED only'() {
        given:
        def renderer = new DotRenderer()
        def graph = new MapperGraph()
        def s = scope('map()')
        ...
        graph.addNode(from)
        graph.addNode(to)
        graph.addEdge(Edge.subSeed(from, to, 'AutoRecurse', Optional.of(Mock(AnnotationMirror))))
        def te = typeElement('com.example.SubSeedLabelMapper')

        when:
        def output = renderer.render(graph, te)

        then:
        def subSeedLine = output.split('\\n').find { it.contains('SUB_SEED') && it.contains('->') }
        subSeedLine != null
        // Label should be just "SUB_SEED" - no strategy name, no weight, no directive
        !subSeedLine.contains('AutoRecurse')
        !subSeedLine.contains('\u221E')
    }
    ...
    def 'REALISED edge with strategyClassFqn shows simple name'() {
        given:
        def renderer = new DotRenderer()
        def graph = new MapperGraph()
        def s = scope('map()')
        ...
        then:
        output.contains('GetterRead')
        !output.contains('io.github.joke.percolate')
    }

    def 'REALISED edge with sentinel weight shows infinity'() {
        given:
        def renderer = new DotRenderer()
        def graph = new MapperGraph()
        def s = scope('map()')
        ...
        graph.addNode(from)
        graph.addNode(to)
        graph.addEdge(Edge.realised(from, to, Weights.SENTINEL_UNREALISED, Optional.empty(), (vars, inputs) -> null, 'ExpensiveStrategy'))
        def te = typeElement('com.example.SentinelMapper')
        ...
        then:
        output.contains('\u221E')
        output.contains('ExpensiveStrategy')
    }

    def 'edge kinds in expanded view are REALISED and SUB_SEED only'() {
        given:
        def renderer = new DotRenderer()
        def graph = new MapperGraph()
        def s = scope('map()')
        ...
        def subSeedFrom = new Node(Optional.of(mockTypeMirror('String')), new SourceLocation(AccessPath.of('e')), s, Optional.empty())
        def subSeedTo = new Node(Optional.of(mockTypeMirror('String')), new TargetLocation(TargetPath.of('f')), s, Optional.empty())
        graph.addNode(realisedFrom)
        graph.addNode(realisedTo)
        graph.addNode(subSeedFrom)
        graph.addNode(subSeedTo)
        graph.addEdge(Edge.realised(realisedFrom, realisedTo, 1, Optional.empty(), (vars, inputs) -> null, 'DirectAssign'))
        graph.addEdge(Edge.subSeed(subSeedFrom, subSeedTo, 'AutoRecurse', Optional.empty()))
        def te = typeElement('com.example.EdgeKindsMapper')

        when:
        def output = renderer.render(graph.expandedView(), te)

        then:
        // REALISED edges show strategy name + weight, not the kind token
        output.contains('DirectAssign (1)')
        output.contains('SUB_SEED')
        // Check for SEED as a standalone token (not as substring of SUB_SEED)
        !output.contains('label="SEED')
