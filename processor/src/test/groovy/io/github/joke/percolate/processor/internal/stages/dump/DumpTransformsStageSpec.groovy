...
@Tag('unit')
class DumpTransformsStageSpec extends Specification {
    ...
    @Subject
    DumpTransformsStage stage = new DumpTransformsStage(writer)

    def ctx = new MapperContext(Stub(TypeElement))

    def 'a mapper with no graph yet dumps an empty (always-false) transforms view'() {
        when:
        stage.run(ctx)

        then:
        1 * writer.dump(ctx, 'transforms') { !it.test(Stub(GraphVertex)) }
        0 * _
    }

    def 'reachable vertices pass the transforms predicate; an orphan does not'() {
        ...
        then:
        1 * writer.dump(ctx, 'transforms') { it.test(root) && it.test(param) && !it.test(orphan) }
        0 * _
    }
}
