...
class ExtractedPlanSpec extends Specification {
    ...
    final Value root = graph.valueFor(scope, new TargetLocation(TargetPath.of('')), STRING, Nullability.NON_NULL)
            .tap { graph.markReturnRoot(it) }

    def 'an OR resolves to the cheapest producer'() {
        given:
        final var param = source('p', STRING)
    ...
    def 'an UNSAT producer never participates, regardless of weight'() {
        given:
        final var param = source('p', STRING)
        final var starved = graph.valueFor(scope, new TargetLocation(TargetPath.of('missing')), STRING, Nullability.NON_NULL)
    ...
    def 'Operation cost sums over all ports (not the minimum over one)'() {
        ...
        final var pa = source('a', STRING)
        final var pb = source('b', STRING)
        final var ia = intermediate('ia', STRING, 3, [pa])
        final var ib = intermediate('ib', STRING, 3, [pb])
        produce(root, 0, [ia, ib]) // weight 0 + 3 + 3 = 6 (min-over-ports would be 3)
        final var ic = intermediate('ic', STRING, 1, [pa])
    ...
    def 'totality dominance counts partials transitively through ports, before cost'() {
        ...
        final var pa = source('a', STRING)
        final var pb = source('b', STRING)
        final var viaPartial = intermediatePartial('ip', STRING, 0, [pa])
        final var viaTotal = intermediate('it', STRING, 8, [pb])
    ...
    def 'reachability is derived from cost: a value reachable from a supply root is reachable, an orphan is not'() {
        ...
        final var param = source('p', STRING)
        produce(root, 1, [param])
        final var orphan = graph.valueFor(scope, new TargetLocation(TargetPath.of('orphan')), STRING, Nullability.NON_NULL)
    ...
    def 'a producerless multi-segment ACCESS source value is unreachable, not a vacuous base case'() {
        ...
        final var orphanAccess = graph.valueFor(
                scope, new SourceLocation(new AccessPath(['p', 'addr'])), STRING, Nullability.NON_NULL)
    ...
    def 'a zero-weight cycle is unreachable (well-foundedness: no value is reachable through its own cycle)'() {
        ...
        final var a = graph.valueFor(scope, new TargetLocation(TargetPath.of('a')), STRING, Nullability.NON_NULL)
        final var b = graph.valueFor(scope, new TargetLocation(TargetPath.of('b')), STRING, Nullability.NON_NULL)
