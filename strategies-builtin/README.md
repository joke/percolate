...
The module depends only on `spi` and uses Google `auto-service` to generate `META-INF/services` service registration files.

Included automatically by `processor`'s `runtimeOnly` declaration — end users get the built-ins transparently on their annotation-processor classpath. Users wanting custom-only setups may `exclude` this artifact:
