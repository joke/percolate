# Directive Reading Spec

## Purpose

Defines `DirectiveReader`, the SPI role that translates a user-facing annotation into the engine's neutral vocabulary. Readers are service-loaded in a deterministic order and see a whole mapper method; they publish bindings, directive inputs, scope inputs, constraints and outright rejections through a `DirectiveSink`, and the discovery stage does nothing but run them and assemble what they declare. Three built-in readers (`@Map`/`@MapList`, `@MapEnum`/`@MapEnumList`, `@Ambient`) cover percolate's own vocabulary, which is what makes a third-party annotation a matter of shipping a reader rather than patching the processor. Readers see only explicitly written annotation members — presence is "written", with the empty string a real value — and each annotation's shape rules belong to the reader that owns it. The engine keeps only its own two structural rules: one binding per target path, and a source path rooted at a scope input.
