...
All `javax.lang.model.type.TypeMirror` instances flowing through any strategy unit spec SHALL originate from the single `com.sun.source.util.JavacTask` held inside `io.github.joke.percolate.spi.test.TypeUniverse` (the fixture published by `percolate-spi`'s `testFixtures` configuration).
