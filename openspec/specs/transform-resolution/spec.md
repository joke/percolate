...
Defines the ResolveTransforms stage that bridges type gaps between source and target properties by resolving transforms (DIRECT for assignable types, SUBMAP for sibling method delegation), producing a ResolvedModel with per-method transform chains for code generation.
