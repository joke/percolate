# Demand Constraints Spec

## Purpose

Defines how a demand's candidate set is narrowed without teaching the engine any strategy semantics. A `DirectiveReader` contributes an opaque, demand-scoped predicate through `DirectiveSink.constrain(targetPath, constraint)`; the engine applies the attached constraints as a conjunction when landing each candidate, branching only on the answer and never inspecting the predicate. A constrained-out candidate leaves a `Refusal` on the demanded `Value` rather than a diagnostic, so contradictory constraints explain themselves instead of collapsing into "no producer". This is the engine's single admissibility mechanism — the self-call rule rides it, no second landing guard exists, and exclusion is unreachable through operation weight.
