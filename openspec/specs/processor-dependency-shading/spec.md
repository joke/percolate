# Processor Dependency Shading Spec

## Purpose

`processor` depends on several internal-only third-party libraries — `org.jgrapht:jgrapht-core`, `com.google.auto:auto-common`, and `com.google.dagger:dagger` — that are consumed exclusively inside `processor` and never cross the spi↔processor boundary. Publishing them as ordinary external dependencies both leaked a "missing dependency version" Maven Central publish failure (their versions were managed only by the internal, unpublished `:dependencies` platform) and exposed percolate to a foreign version of any of them colliding with percolate's own on a consumer's shared annotationProcessor classpath — the same risk the `javapoet-relocation` capability addresses for JavaPoet. This spec defines how `processor` shades and relocates these libraries (and their own internal-only runtime dependencies) directly into its own published jar under `io.github.joke.percolate.lib.*`, fully swallowing them.
