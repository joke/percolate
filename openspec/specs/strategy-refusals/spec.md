# Strategy Refusals Spec

## Purpose

Defines the three-valued answer an `ExpansionStrategy` gives: a production, an empty stream meaning "not mine", or a `Refusal` carrying a message and an opaque `Subject`. A refusal is data, not a side effect — strategies never touch the `Messager`. Refusals are anchored on the demanded `Value` as negative space beside its producers, and the realisation renderer surfaces them at the deepest unsatisfied miss in place of the generic "no producer" line, so a strategy that recognised a demand but could not serve it explains why. `Subject` is an opaque position handle the SPI alone constructs, keeping javax positioning out of strategy code, and a type variable may carry a `Bound` whose refused grounding instantiates no spec and records its reason the same way.
