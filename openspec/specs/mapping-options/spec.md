# Mapping Options Spec

## Purpose

Defines the `MapOptKey` enum, `@MapOpt` annotation, and extensions to `@Map` that allow per-mapping options (such as date format patterns) to flow from annotation declarations through `MapDirective` and `MappingEdge` to code generation strategies.
