...
Defines how percolate's published artifacts reach Maven Central. Publishing happens only in CI, gated strictly on a release-versioning-created release (see the `release-versioning` capability) — never on an ordinary push and never as a `-SNAPSHOT`. Every publishable module targets the Central Portal API exclusively via a single aggregated deployment, signs every artifact, and carries declarative, uniformly-applied POM metadata plus sources/javadoc jars.
