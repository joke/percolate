# Nested Source Chain Spec

## Purpose

Defines how dot-separated source strings in `@Map` annotations are parsed into multi-segment source property chains in the symbolic graph, how shared prefix nodes are reused, and how `ResolveTransformsStage` resolves each segment sequentially by performing property discovery at each level.
