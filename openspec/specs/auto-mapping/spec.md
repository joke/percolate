...
Defines the automatic property mapping behavior in `BuildGraphStage`, where source and target properties with the same name are connected by `MappingEdge` instances without requiring explicit `@Map` directives.
