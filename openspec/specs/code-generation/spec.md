...
### Requirement: GenerateStage produces JavaFile from validated graph
The `GenerateStage` SHALL walk the validated mapping graph and produce a `JavaFile` using Palantir JavaPoet. It SHALL support constructor-based and field-based target construction based on the `WriteAccessor` types present in the graph.

#### Scenario: Constructor-based target generation
- **WHEN** the target type's properties are all `ConstructorParamAccessor` write accessors
- **THEN** the generated code SHALL gather mapped values and pass them as constructor arguments in parameter order

#### Scenario: Field-based target generation
- **WHEN** the target type's properties are all `FieldWriteAccessor` write accessors
