...
The group's view SHALL NOT auto-grow when REALISED edges are added to the underlying graph outside the group's direct slot-incoming chain. Deeper source-side chain edges live in the underlying graph but are NOT members of the group's view. This keeps each group focused on its codegen's direct inputs.

`ExpansionGroup` SHALL expose: `getRoot()`, `getSlots()`, `getCodegen()`, `getStrategyClassFqn()`, `getView()`, and `contains(Edge e)`.

#### Scenario: of() rejects a root not in the underlying graph
- **WHEN** `ExpansionGroup.of(root, slots, codegen, "com.example.X", initialEdges, parent)` is invoked with a `root` that has not been added to `parent.underlyingGraph()`
- **THEN** an `IllegalArgumentException` is thrown
- **AND** no group is constructed
...
#### Scenario: View is scoped to root, slots, and initialEdges
...
#### Scenario: View does not auto-grow with unrelated REALISED edges
- **WHEN** a group is constructed and a new REALISED edge is added to `parent.underlyingGraph()` whose endpoints are not in `group.getView().vertexSet()`
- **THEN** `group.getView().edgeSet()` does NOT contain the new edge
- **AND** `group.getView().vertexSet()` is unchanged
