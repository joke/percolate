...
This spec defines `@Ambient`, the parameter annotation that threads a value through a mapper's plan by key
rather than by structural descent: a top-level `@Ambient` parameter is supplied by the caller and published
into its scope's ambient environment, and a nested method's own `@Ambient` parameter of the same key binds
from that environment and republishes it for its own subtree. It covers the annotation itself, the bind/publish
behaviour, key-based binding with type verification, duplicate- and unbound-key error reporting, how
`MethodCallBridge` emits ambient ports, and that an `@Ambient` parameter remains an ordinary `@Map` source root.
