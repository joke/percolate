...
**Reason**: `hasErrorsFor(Element)` conflated a diagnostic's **position** with its **attribution**, and
approximated attribution with a one-level `Element.getEnclosingElement()` containment heuristic. Because both
travelled in one argument, positioning a message at the element that is actually wrong could silently break
suppression and let code generation proceed for a broken mapper — which is why ambient diagnostics were
positioned at the mapper type rather than at the offending parameter. Attribution is now exact: a diagnostic
belongs to the mapper whose pipeline recorded it.

**Migration**: `diagnostics.hasErrorsFor(ctx.getMapperType())` becomes a query on the mapper's own collected
diagnostics. Callers that positioned a diagnostic at the mapper type solely to satisfy the containment
heuristic SHALL position it at the offending token instead.

### Requirement: Diagnostics state SHALL reset between processor rounds

**Reason**: Per-round `reset()` existed because scarring lived in a `@Singleton` shared across every mapper of
a round. Diagnostics are now collected on the per-mapper context, which is created fresh for each mapper on
each round, so there is no cross-mapper or cross-round state to clear. Removing it also deletes global mutable
state from the processor.

**Migration**: No caller replacement is needed. `MapperStep` no longer invokes `reset()`; the per-mapper
context supersedes it.
