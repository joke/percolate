...
- [ ] 3.6 Migrate all unit specs off `TypeUniverse`/mirrors: 16 `processor` specs + ~30 `strategies-builtin` specs + `reactor`/`reactor-blocking` specs (use `TestTypes.of`/builders/constants; construct directive values directly); update `ResolveCtxBuilder`
...
- [ ] 4.2 Remove `@Isolated` from all 16 specs
