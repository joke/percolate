...
flowchart LR
    ...
    E --> F["./gradlew check"]
    F --> G["./gradlew publishToMavenCentral"]
```
...
`spi` imported the internal `:dependencies` platform via both `implementation platform(project(':dependencies'))` and `api platform(project(':dependencies'))`. Applying required deleting both, not just narrowing `api` to `implementation` as first assumed: `spi`'s `implementation` scope had no other dependency actually needing the platform's managed versions (it was dead weight), so it was removed outright rather than kept at a lower scope. A second, undocumented leak source surfaced during implementation: `java-test-fixtures` variants are flattened into the *same* published Maven POM as the main component (confirmed empirically — `spock-core`/`groovy`, declared only on `testFixturesImplementation`, already appeared in the POM before this fix), so `testFixturesImplementation platform(project(':dependencies'))` had to be removed too (compile-time version resolution for its dependencies still comes from the retained `testFixturesCompileOnly platform(...)`, which participates in `compileClasspath` resolution but not the published `runtimeElements` variant). With every platform edge feeding a published variant gone, the `pom.withXml { asNode().dependencyManagement.each { it.parent().remove(it) } }` workaround was deleted — verified by inspecting the generated POM directly, not just by removing the code.
...
### Javadoc jars require delombok, not just doclint suppression

Turning on `withJavadocJar()` (task 5.3) revealed `javadoc` had never actually run on this codebase. Two distinct Lombok/javadoc incompatibilities surfaced, not one: (1) a `{@link #getInputType()}` reference to a Lombok-generated accessor in `spi`, which javadoc's parser can't see since it only reads source, not Lombok's bytecode output; (2) `@RequiredArgsConstructor(onConstructor_ = @Inject)` — Lombok's mechanism for putting an annotation on a generated constructor, a compile-time-only AST rewrite, not a real declared annotation element — which is unresolvable under javadoc's own front end, producing hard `cannot find symbol` errors (not warnings) across 23 sites in `processor`. Suppressing doclint (`-Xdoclint:none`) and setting `failOnError = false` silenced the second class of error but left `processor`'s javadoc jar empty (javadoc aborted generation entirely rather than skipping just the broken files) — a technically-present but practically-useless deliverable.

Presented three options to the user (ship empty jars, add a delombok plugin, rewrite the affected source): they chose adding `io.freefair.gradle:lombok-plugin` (`io.freefair.lombok`, version pinned to `1.18.46` to match the existing `:dependencies`-platform-pinned version — the plugin's own default, `1.18.44`, would have silently downgraded it). Applying it to every Lombok-using module wires `javadoc` to compile against **delomboked** (expanded, Lombok-free) source automatically, which resolves both problems at their root — real methods exist in the delomboked source for `{@link}` to find, and the constructor's `@Inject` annotation is already a plain, real annotation by the time javadoc parses it. Result: fully populated javadoc jars (159 files / ~5.9MB for `processor`, 108 / ~5.3MB for `spi`), no doclint suppression needed.

- **Discovered mid-fix**: the plugin's own `extendsFrom`-based dependency wiring (which was meant to let every module's manual `annotationProcessor`/`compileOnly 'org.projectlombok:lombok'` declaration be deleted) is not sufficient on its own — `processor` (which also runs `dagger-compiler`) failed with `Dagger/MissingBinding: MapperStep cannot be provided without an @Inject constructor`, reproducible on a clean build, until Lombok was re-declared explicitly *first* in each module's `annotationProcessor`/`compileOnly` blocks. The plugin's automatic wiring appends Lombok's jar later in the resolved annotation-processor path than an explicit early declaration does, and — contrary to Lombok's usual "activation order shouldn't matter" self-priority mechanism — that changed effective behavior here. The explicit declarations were restored in every affected module (`spi`, `processor`, `strategies-builtin`, `reactor`, `reactor-blocking`) rather than relying on the plugin's own wiring alone.
- **Alternative considered**: ship empty/near-empty javadoc jars (doclint:none + failOnError=false only). Rejected by the user in favor of the proper fix.
- **Alternative considered**: rewrite the 23 affected `onConstructor_` call sites and the 1 broken `{@link}` in production source. Rejected — out of scope for a build/publishing change, and risks subtly altering DI-wiring behavior in the core engine.
