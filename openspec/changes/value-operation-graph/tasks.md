...
- [ ] 3.1 Rewrite `SeedStage`: parameter-root and return-root `Value`s only; no edges, groups, or target leaves
- [ ] 3.2 Derive per-level declared-bindings goal specs from dotted `@Map` target paths (constants/defaults included as bindings)
- [ ] 3.3 Attach goal specs to demands; remove `SEED` edge kinds and seed-group registration
...
- [ ] 4.1 Implement the demand work-list over unsatisfied `Value`s (target-to-source, per-pass immutable snapshot, batch apply at pass boundary)
- [ ] 4.2 Implement Horn SAT propagation (Value: any producer; Operation: all ports + child return-root; bases: param roots, zero-port Operations) as a memoized vertex predicate
- [ ] 4.3 Rework `FrontierMatcher`: spec → atomic `AddOperation`, per-port demand fan-out, signature dedup; delete `InputAllocator` fall-through (no silent sourcing)
- [ ] 4.4 Implement the goal-spec gate in assembly matching (constructor candidate iff param-name set equals declared-children set)
- [ ] 4.5 Carry the binding directive in the demand context; delete `Node.inheritDirective` successor paths
- [ ] 4.6 Conversion chains as unary Operations over `valueFor`-deduped intermediates (type-keyed, bounded, stop-at-SAT)
- [ ] 4.7 Rework `Applier` to `AddValue`/`AddOperation` with assertion-only cycle check (D10 gate); delete rollback
- [ ] 4.8 Delete `ExpansionGroup`, `GroupId`, `GroupOutcome`, group expanders/phases, group snapshots
...
- [ ] 5.1 Implement bottom-up cost extraction (min over SAT producers, weight + sum over ports) with deterministic tie-break
- [ ] 5.2 Implement the read-only plan view (single `chosenProducer()` per in-plan Value; losers untouched); recurse through scope-owning Operations
- [ ] 5.3 Delete `PlanView` (three pruning passes), Dijkstra cost oracle, `RealisedSubgraph`
...
- [ ] 6.1 Rewrite `BuildMethodBodies` as the plan walk (chosen producer, port-keyed `IncomingValues`, recursive operand rendering)
- [ ] 6.2 Render scope-owning Operations as container weave around the child-plan lambda (handles attach to Operations)
- [ ] 6.3 Delete `applyNullabilityContract`, consumer-contract resolution, `NullabilityResolver` injection from generation
- [ ] 6.4 Verify generated output unchanged across the integration test suite (the behavioural contract)

## 7. Nullness and defaults as Operations (design D5)

- [ ] 7.1 Emit `[requireNonNull]` Operations on `NULLABLE → NON_NULL` crossings (existing message format) at expansion time
- [ ] 7.2 Emit `[coalesce]` instead when the binding directive declares `defaultValue` (ternary / `orElse` forms, constant coercion reuse)
- [ ] 7.3 `ConstantValue` as zero-port Operation producing a `NON_NULL` Value
...
- [ ] 8.1 Rework `RealisationDiagnosticsStage`: walk unsatisfied demands; delete `UNSAT_DID_NOT_CONVERGE`
- [ ] 8.2 Implement closest-miss as the deepest unsatisfied port chain (fewest-unsatisfied-ports producer, named ports, no-producer leaf)
- [ ] 8.3 Rework `DotRenderer`/dump stages: Operation boxes, Value ellipses, port-labelled edges, child-scope clusters; plan dump from the extraction view
...
- [ ] 9.2 Delete all dead types and grep-verify: no `ExpansionGroup`, `GroupId`, `GroupOutcome`, `EdgeKind`, `ElementScope` (edge attribute), `ExpansionStep`, `Slot` (edge-carried), `PlanView` references remain
- [ ] 9.3 Confirm `BuildMethodBodies` references no group/label surface (grep) and diff generated mappers before/after for a representative integration mapper
- [ ] 9.4 Run `./gradlew check` — NEVER continue if there are violations
