...
- **Containers become scope-owning Operations**: the per-element transform is a child scope (param root in, return root out — the same shape as a method body) in the same flat graph. Invariant: no dependency edge crosses a scope boundary; the only parent/child coupling is the owning Operation. The `ElementScope` `ENTERING`/`EXITING` edge attribute is removed; wrap (`Optional.of`) is a plain Operation, structurally distinct from element-mapping.
...
- **Not changing:** the public `@Map` annotation surface and the generated mapper output. The integration test suite is the behavioural contract.
...
- `container-expansion`: container strategies emit scope-owning Operations (element mapping) or plain Operations (wrap/unwrap); the strictly-linear REALISED-chain invariant is replaced by the scope-ownership invariant.
...
- **Tests** — harness fixtures and strategy unit specs updated to the new surfaces; DOT goldens regenerated; **integration tests must pass with unchanged generated output** (the behavioural contract for the whole change).
