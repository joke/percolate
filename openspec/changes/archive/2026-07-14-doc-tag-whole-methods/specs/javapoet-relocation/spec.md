...
The project SHALL provide a Gradle module `percolate-javapoet` that repackages `com.palantir.javapoet:javapoet:0.16.0` by relocating the `com.palantir.javapoet` package to `io.github.joke.percolate.javapoet`. The relocated jar SHALL be the module's consumable `api` artifact. The relocation SHALL rewrite the `com.palantir.javapoet` package **only**; the JDK packages the JavaPoet API references (in particular `javax.lang.model.*`) SHALL remain unrelocated so the codegen API still interoperates with the compiler's mirror types. The module MAY additionally carry a minimal relocated **source overlay** (see "Relocated JavaPoet class overlay"); apart from any overlaid class, its classes derive entirely from the upstream artifact.
...
- **WHEN** the shaded `percolate-javapoet` jar is inspected for the overlaid class (`io.github.joke.percolate.javapoet.MethodSpec`)
