...
### Requirement: Assertion scope is metadata-only in this change
...
Where a unit spec discovers behaviour that is unintentional or worth questioning but lies outside the scope of this change to fix, the spec SHALL pin the current behaviour with a feature method whose name signals the pinning intent (e.g. `'pins current behaviour: ListMap accepts Optional inputs'`).
...
The two findings already identified in design (see `design.md` § D8) SHALL be pinned at minimum: `ListMap` accepting `Optional` inputs in `ListMapSpec`, and `MethodCallBridge.subtypeDistance` returning `0` for both same-type and not-assignable inputs in `MethodCallBridgeSpec`.
