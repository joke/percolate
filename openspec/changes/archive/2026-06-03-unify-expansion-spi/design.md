...
- **Combinatorial iteration moves into strategies** → the `CombinatorialMatch` mixin supplies the loop; candidate scoping stays central in the driver, so no strategy hand-rolls scoping. *Consequence found during cutover:* a container's target-driven branches (wrap, unwrap-by-synthesis) ignore the candidate `from`, so the per-candidate loop emits the same step once per candidate. The driver therefore **deduplicates structurally-identical emitted steps** within one frontier round (signature = strategy | intent | scope | output | input types); `from`-dependent conversions differ by input type and are untouched. Without this, identical equal-cost twin sub-groups survive into the graph until the plan oracle prunes them.
- **Whole-graph expansion shares one driver state** → every method's groups expand in one fixed-point loop, so any base case keyed on a process-global "current method" only satisfies one method. The **parameter-root base case resolves against the node's own `MethodScope`** instead, keeping multi-method mappers correct regardless of which method's seed edge is visited first.
