...
**Reason**: The `processor-test-support` gradle module had no concrete consumer. External strategy authors write mapper-level integration tests in their downstream projects rather than building seed graphs against an internal harness, and internal engine tests do not need a published API. Maintaining a separate module for a hypothetical audience was uncompensated overhead.
**Migration**: All surviving test helpers (`TypeUniverse`, `GraphFixtures`, `ExpansionAssertions`, `ExpansionHarness`) move to `processor/src/test/groovy/io/github/joke/percolate/processor/test/` (converted to Groovy sources). Tests that previously depended on `testImplementation project(':processor-test-support')` reference the inlined helpers directly. The module's gradle directory, `build.gradle`, `README.md`, and the build-time tasks specific to it (`checkNoInternalJavacImports`, the `META-INF/services` rule, the per-module Jacoco override) are deleted.

### Requirement: Fluent seed-graph DSL
**Reason**: `SeedDsl` built malformed seed graphs (directive target nodes were untyped, so production strategies could never match) and its load-bearing audience — external strategy authors — turned out not to write tests at this level. Engine tests construct `MapperGraph` instances directly via `GraphFixtures` plus typed `Node`/`Edge.elementSeed(...)` calls, which is more transparent than a fluent surface that hides what gets produced.
**Migration**: Replace `SeedDsl.seed().method(...).arg(...).returns(...).directive(...).build()` with direct `MapperGraph` construction. Use `GraphFixtures` for common shapes (identity seeds, disjoint-pair seeds, cyclic seeds). The `MapperSpec`, `StrategyBundle`, and `PropertyTestBase` value types/classes (whose only job was feeding the DSL through jqwik generators) are removed alongside it.
...
The processor module SHALL contain jqwik-driven property specs (Groovy sources) at `processor/src/test/groovy/io/github/joke/percolate/processor/stages/expand/properties/` covering the algebraic contract of `expand`. Each spec is a Groovy class named `*Spec.groovy` whose methods carry jqwik `@Property` annotations — Spec naming matches the rest of the test layer, jqwik provides the property-test engine. The seven specs are:
...
Generators SHALL produce `MapperGraph` instances directly (via `GraphFixtures` or inline construction) constrained to `TypeUniverse`. Generators SHALL NOT route through a `MapperSpec` / `SeedDsl` indirection. jqwik `@Provide` methods (or `Arbitraries`-backed Groovy helpers) SHALL be the source of randomised inputs; plain `java.util.Random` is not a substitute because it cannot reproduce a shrunk counterexample from a pinned seed.
...
`processor/build.gradle` SHALL configure jqwik with a default `@Property(tries = 500)` (overridable per property) and store a property database under `build/jqwik-database` so that shrunken counterexamples are replayed deterministically. Each property spec SHALL pin a default seed via `@Property(seed = ...)` to make CI failures locally reproducible. The default-tries configuration MAY live as a class-level `@PropertyDefaults` annotation on a small local base class in `processor/src/test/groovy/.../properties/`, on individual property specs, or in `jqwik.properties` — implementation choice. Any such base class SHALL be local to processor's test sources, not a published API.
...
- **WHEN** any property spec under `expand/properties/` is inspected
