...
The new types `CallableMethods`, `MethodCandidate`, `Receiver`, `ThisReceiver` SHALL reside in `io.github.joke.percolate.processor.spi`. They SHALL NOT import any class from `io.github.joke.percolate.processor.graph` or `io.github.joke.percolate.processor.stages.*`. The package boundary established by the prior expansion change is preserved.

The package SHALL declare `@NullMarked` via `package-info.java` (already declared; the new types simply respect the existing declaration).
