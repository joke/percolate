...
**Reason**: The sequence/wrapper split is replaced by a single `Container` base whose kind is emergent
from which operations it supplies (collect ⇒ sequence). The asymmetry survives as "absence of
`collect`", not two base types.
**Migration**: See ADDED "A single Container base; kind is emergent" and MODIFIED "Container-codegen
handle family".
