...
- **BREAKING (engine internal)**: `ExpandGroupsPhase` work-list ordering switches from FIFO to topological over the shared-boundary group dependency DAG (a group `G` depends on group `H` iff `H.root ∈ G.slots`). Dependencies expand first. No fixed-point loop.
...
- `graph-expansion`: candidate search is sub-group-view-scoped; every bridge/resolver match spawns its own one-slot sub-group; topological work-list ordering replaces FIFO; ambient source-parameter-roots and global cycle-prevention are removed; base-case SAT for parameter-root slots is determined structurally per-group.
