...
- [ ] 4.6 Replace the FIFO work-list with topological iteration over the boundary DAG. Implement as: at each iteration, scan registered groups for the first whose dependencies (groups whose root is in this group's slots) are all already drained. Re-scan after each new sub-group registration (sub-groups are leaves; insert at the head of the work-list for next-iteration processing).
