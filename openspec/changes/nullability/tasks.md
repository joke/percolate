...
- [ ] 8.1 Update `ExpandGroupsPhase.registerNestedGroupTarget` (line 524) to create slot Nodes with `Optional.empty()` for both `type` and `nullability` — drop the `Optional.of(slot.getType())` argument. **Deferred:** nested slot Nodes drive recursive expansion via `resolveSlot`, which reads `slot.getType()`. Making them untyped requires a separate refactor to surface `Slot.type` to the engine (e.g., via `ExpansionGroup.slotMetadata`). Tracked as follow-up.
