...
This invariant exists so that the `.dot` debug outputs produced by `DumpGraph` / `DumpExpandedGraph` / `DumpTransforms` faithfully reflect the graph that `GenerateStage` consumed. Future graph-modifying stages (e.g., an optimisation pass) SHALL be ordered before `dump` in the pipeline so the same invariant holds.
...
- **THEN** every graph-dumping stage (`DumpGraph`, `DumpExpandedGraph`, plus any future `Dump*` stage) appears strictly before `GenerateStage`
