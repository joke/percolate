...
- [x] 7.1 `./gradlew spotlessApply` + `./gradlew check --max-workers=1` → BUILD SUCCESSFUL (all modules; reactor's own `:reactor:check` green: spotless/PMD/errorprone/NullAway/codenarc/tests). NOTE: the default *parallel* `./gradlew check` is now flaky (nondeterministic javac-internals re-entrancy across the larger module graph on the Java 25 runtime — different modules fail per run); `--max-workers=1` is deterministically green. Constraining parallelism for the javac-based quality tasks is a candidate build follow-up.
