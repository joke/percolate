...
/**
 * {@link JustOrEmpty} unit-tested mock-only over the {@link ResolveCtx} type-query seam: every seam question is
 ...
 */
@Tag('unit')
class JustOrEmptySpec extends Specification {

    ResolveCtx ctx = Mock()
    TypeMirror to = Mock()
    TypeMirror elementType = Mock()
    TypeElement optionalElement = Mock()
    TypeMirror optionalOfElement = Mock()
    ...
    def 'declines when java.util.Optional is not on the compile classpath'() {
        when:
        def specs = new JustOrEmpty().expand(Demands.forTarget(to), ctx).toList()
        ...
        1 * ctx.typeElementNamed('java.util.Optional') >> null
        0 * _
    ...
    def 'emits a justOrEmpty operation bridging a concrete Optional<T> into Mono<T>'() {
        ...
        1 * ctx.isType(to, Reactors.MONO) >> true
        1 * ctx.typeArgument(to, 0) >> elementType
        1 * ctx.typeElementNamed('java.util.Optional') >> optionalElement
        1 * ctx.declaredType(optionalElement, elementType) >> optionalOfElement
        0 * _

        expect:
        specs.size() == 1
        def spec = specs[0]
        spec.label == 'justOrEmpty'
        spec.codegen instanceof OperationCodegen
        spec.weight == Weights.CONTAINER
        spec.childScope.empty
        spec.ports.size() == 1
        spec.ports[0].name == 'optional'
        spec.ports[0].type.is(optionalOfElement)
        ...
        ((OperationCodegen) spec.codegen).render(singleInput(CodeBlock.of('$N', 'source'))).toString() ==
                'reactor.core.publisher.Mono.justOrEmpty(source)'
    }
